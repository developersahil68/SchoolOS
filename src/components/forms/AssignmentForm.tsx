...
const AssignmentForm = ({
  ...
  // Get the classId from the lesson if updating
  useEffect(() => {
    if (data?.lessonId && lessons.length > 0) {
      const lesson = lessons.find(
        (lesson: { id: number; classId: number }) =>
  ...
  const filteredLessons = selectedClassId
    ? lessons.filter(
        (lesson: { id: number; name: string; classId: number }) =>
          lesson.classId === parseInt(selectedClassId as string)
      )
    : lessons;
